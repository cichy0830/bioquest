...
function submittedState(mode) {
  ...
  return {
    ...
    attempt_id: `${mode}_u34_attempt`,
    attempt_session_token: `${mode}_u34_token`,
    attempt_session_id: `${mode}_u34_session`,
    ...
    result: {
      ...
      earned_badges: verified ? ["server_u34_badge_without_image"] : ["abo_blood_type_entry", "abo_blood_type_flawless"],
      logs: [],
      reflection: {}
    },
    submitted: true,
...
async function runCase(browser, mode, viewport) {
  ...
  await page.addInitScript(({ mode, state }) => {
    localStorage.setItem("bioquest_abo_blood_type_state_v1", JSON.stringify(state));
    localStorage.setItem("bioquest_attempts_v1", JSON.stringify([{ unit_id: "abo_blood_type", attempt_id: "historic_attempt", verification_status: "server_verified" }]));
    window.confirm = () => true;
  ...
  const cleanState = await page.evaluate(() => JSON.parse(localStorage.getItem("bioquest_abo_blood_type_state_v1")));
...
console.log("abo_blood_type submitted layout regression passed");
