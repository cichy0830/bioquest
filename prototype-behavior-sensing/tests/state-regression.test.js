...
const root = process.env.BIOQUEST_AUDIT_ROOT
  ...
  : sourceRoot;
const VERSION = "20260813-behavior-sensing-mapping-v1";
...
function studentForStatus(status) {
  ...
  return {
    ...
    class_name: "723",
    seat_no: "23",
    student_name: verified ? "行為感應測試生" : "待確認學生",
    profile_gender: "male",
    total_exp: verified ? 10800 : 10400,
    ...
    progress: {
      ...
      total_exp: verified ? 10800 : 10400,
      completed_unit_count: verified ? 23 : 22,
      current_title_id: "systems_investigator",
      title_avatar_path: "../shared-assets/title-avatars/title-06-systems_investigator-male.webp",
      unit_badge_summary_json: JSON.stringify([
        { unit_id: "endocrine_system", earned_count: 0, total_count: 15, earned_badges: [] },
        { unit_id: "behavior_sensing", earned_count: 0, total_count: 15, earned_badges: [] }
      ])
    }
  };
}
  ...
  for (const viewport of [{ width: 1440, height: 900 }, { width: 390, height: 844 }]) {
    for (const status of ["verified", "pending", "guest"]) {
      ...
      await page.addInitScript(({ seededState, attemptsKeyName, stateKeyName, canonicalVersion }) => {
        ...
        localStorage.setItem(attemptsKeyName, JSON.stringify([{ attempt_id: "history_attempt", unit_id: "behavior_sensing", unit_credited_exp: 500 }]));
        ...
        window.fetch = async (url, options = {}) => {
          ...
          return { ok: true, json: async () => ({
            ...
            progress: {
              ...
              current_title_id: "systems_investigator",
              title_avatar_path: "../shared-assets/title-avatars/title-06-systems_investigator-male.webp",
              unit_badge_summary_json: JSON.stringify([{ unit_id: "endocrine_system", earned_count: 0, total_count: 15, earned_badges: [] }])
            }
          }) };
        };
      ...
      if (status === "verified") {
        assert(achievementState.text.includes("10800 EXP"), "verified title progress must come from StudentProgress");
        assert(achievementState.text.includes("已完成 23 站"), "verified completed units missing");
      }
      if (status === "pending") {
      ...
      const resetState = await page.evaluate(() => ({
        screen: window.__behavior_sensingTest.state().screen,
        student: window.__behavior_sensingTest.state().student,
        attemptId: window.__behavior_sensingTest.state().attempt_id,
        submitted: window.__behavior_sensingTest.state().submitted,
        attempts: window.__behavior_sensingTest.loadAttempts().length,
        snapshot: window.__behavior_sensingTest.loadVerifiedSnapshot(),
      ...
      if (status === "guest") {
        ...
        assert.equal(resetState.snapshot.student_id, "S23023", `${status} snapshot student`);
        assert.equal(resetState.snapshot.total_exp, status === "verified" ? 10800 : 10400, `${status} snapshot total`);
        await page.locator("#studentId").fill("S23023");
...
console.log("behavior sensing submitted retry state regression passed");
