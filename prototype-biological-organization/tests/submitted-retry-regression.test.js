...
const CACHE = "20260731-biological-organization-submitted-retry-ia-v1";
const QUESTION_VERSION = "20260716-biological-organization-canonical-v1";
  ...
  return JSON.stringify([
    ...
    },
    {
      ...
      earned_badges: [
        ...
        { badge_id: "biological_organization_reflection_reporter", badge_image_path: "shared-assets/badges/biological_organization/badge-biological_organization-biological_organization_reflection_reporter.webp" }
      ]
    }
  ]);
}
...
function progressFor(mode) {
  ...
  return {
    ...
    total_exp: mode === "verified" ? 4320 : 3880,
    completed_unit_count: mode === "verified" ? 9 : 8,
...
function resultFor(mode) {
  ...
  return {
    ...
    attempt_total_exp_candidate: verified ? 500 : 410,
    ...
    badges: verified
      ? ["biological_organization_entry", "organization_hierarchy_sorter", "biological_organization_reflection_reporter"]
      : ["biological_organization_entry", "organization_hierarchy_sorter"]
  };
}
...
function submittedState(mode) {
  return {
    ...
    remote_completed_attempts: mode === "guest" ? 0 : 8,
    remote_previous_attempt_id: mode === "guest" ? "" : "u9_previous_attempt",
    remote_previous_accuracy: mode === "guest" ? null : 0.78,
    cumulative_badges: mode === "verified" ? ["biological_organization_entry", "organization_hierarchy_sorter", "biological_organization_reflection_reporter"] : [],
    cumulative_total_exp: mode === "verified" ? 4320 : 0,
    completed_unit_count: mode === "verified" ? 9 : 0,
    started_at: "2026-07-31T00:00:00.000Z",
    attempt_id: `u9_${mode}_submitted_attempt`,
    attempt_session_id: `u9_${mode}_submitted_session`,
    attempt_session_token: mode === "guest" ? "" : `u9_${mode}_submitted_token`,
    ...
    answers: {
      q01_sequence: ["cell", "tissue", "organ", "system", "individual"],
      q02: {},
      q10: {},
      q12: {},
      reflection: {}
...
async function installBackend(page, mode) {
  await page.addInitScript(({ mode, questionVersion }) => {
    ...
    window.fetch = async (rawUrl) => {
      ...
      window.__u9RetryActions.push({ action, url: String(url) });
        ...
        return response({
          ...
          progress: {
            ...
            unit_badge_summary_json: JSON.stringify([
              { sequence: 9, unit_id: "biological_organization", earned_count: 3, catalog_count: 10, earned_badges: [] }
            ])
          },
          attempt_status: {
            completed_attempt_count: 9,
            previous_attempt_id: "u9_previous_attempt",
      ...
      if (action === "startAttempt") {
        return response({
          ...
          attempt_type: "retry",
          attempt_id: `u9_${mode}_new_attempt`,
          attempt_session_id: `u9_${mode}_new_session`,
          attempt_session_token: `u9_${mode}_new_token`,
          previous_attempt_id: `u9_${mode}_submitted_attempt`,
...
async function runCase(browser, viewport, mode) {
  ...
  await page.addInitScript((stateSnapshot) => {
    localStorage.clear();
    localStorage.setItem("bioquest_biological_organization_state_v1", JSON.stringify(stateSnapshot));
    localStorage.setItem("bioquest_attempts_v1", JSON.stringify([
      {
        attempt_id: "history_attempt",
        mission: { unit_id: "biological_organization" },
  ...
  const resetState = await page.evaluate(() => ({
    screen: window.__biologicalOrganizationTest.state().screen,
    student: window.__biologicalOrganizationTest.state().student,
    result: window.__biologicalOrganizationTest.state().result,
    submittedAt: window.__biologicalOrganizationTest.state().submitted_at,
    attemptId: window.__biologicalOrganizationTest.state().attempt_id,
    sessionId: window.__biologicalOrganizationTest.state().attempt_session_id,
    sessionToken: window.__biologicalOrganizationTest.state().attempt_session_token,
  ...
  if (mode === "guest") {
    ...
    await page.locator("#studentIdInput").fill("S70109");
  ...
  const reloginState = await page.evaluate(() => ({
    ...
    sessionId: window.__biologicalOrganizationTest.state().attempt_session_id,
    sessionToken: window.__biologicalOrganizationTest.state().attempt_session_token,
    previousAttemptId: window.__biologicalOrganizationTest.state().remote_previous_attempt_id,
    questionVersion: window.__biologicalOrganizationTest.state().question_version,
    submittedAt: window.__biologicalOrganizationTest.state().submitted_at,
    resultPresent: Boolean(window.__biologicalOrganizationTest.state().result),
    backendStatus: window.__biologicalOrganizationTest.state().backend_status,
    actions: window.__u9RetryActions || []
  }));
  ...
  if (mode === "guest") {
    ...
    assert.equal(reloginState.attemptId, `u9_${mode}_new_attempt`, `${mode} ${viewport.width}: formal relogin must create a new attempt`);
    assert.equal(reloginState.sessionId, `u9_${mode}_new_session`, `${mode} ${viewport.width}: formal relogin must create a new session`);
    assert.equal(reloginState.sessionToken, `u9_${mode}_new_token`, `${mode} ${viewport.width}: formal relogin must create a new token`);
    assert.equal(reloginState.previousAttemptId, `u9_${mode}_submitted_attempt`, `${mode} ${viewport.width}: formal relogin must carry previous attempt id`);
...
(async () => {
  ...
  console.log("biological_organization submitted retry regression passed");
})();
