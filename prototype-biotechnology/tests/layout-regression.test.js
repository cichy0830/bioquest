...
function submittedState(mode) {
  ...
  return {
    ...
    attempt_id: `${mode}_u36_attempt`,
    attempt_session_token: `${mode}_u36_token`,
    attempt_session_id: `${mode}_u36_session`,
    ...
    answers: {},
    hints: {},
    reflection: { confident: "資料支持範圍", question: "我想確認農業生技資料表能支持到哪個判斷範圍？", confidence: "4" },
    result: {
      ...
      earned_badges: verified ? ["server_u36_badge_without_image"] : ["biotechnology_entry", "biotechnology_flawless"],
      logs: [],
      reflection: {}
    },
    submitted: true,
...
async function runCase(browser, mode, viewport) {
  ...
  await page.addInitScript(({ mode, state }) => {
    localStorage.setItem("bioquest_biotechnology_state_v1", JSON.stringify(state));
    localStorage.setItem("bioquest_attempts_v1", JSON.stringify([{ unit_id: "biotechnology", attempt_id: "historic_attempt", verification_status: "server_verified" }]));
    window.confirm = () => true;
  ...
  const cleanState = await page.evaluate(() => JSON.parse(localStorage.getItem("bioquest_biotechnology_state_v1")));
...
try {
  for (const viewport of viewports) {
    ...
    await runEvidenceCase(browser, viewport, "checkpoint3", ".biotech-data-evidence", ["農業生技資料", "B1", "C1", "受害葉片比例"], /B1 較有效|基因改造比較好|答案是|先排除/);
    await runEvidenceCase(browser, viewport, "checkpoint3", ".biotech-article-excerpt", ["文章摘句卡", "這項技術保證百分之百安全"], /這句話誇大|不可信|注意保證語氣|沒有證據|答案是/);
    await runEvidenceCase(browser, viewport, "checkpoint2", ".biotech-comparison-cards", ["生技案例比較卡", "案例一", "案例二"], /複製案例|基因轉殖案例|第一張是複製|第二張是基因轉殖|答案是/);
  }
} finally {
  await browser.close();
}

console.log("biotechnology submitted layout regression passed");
