...
const VERSION = "20260727-cardiovascular-components-relogin-v1";
...
const verifiedSnapshotKey = "bioquest_cardiovascular_components_verified_snapshot_v1";
...
function resetScreenScroll() {
  ...
  const scrollTop = () => {
    for (const target of scrollTargets) {
      if (target === window && typeof window.scrollTo === "function") window.scrollTo({ top: 0, left: 0, behavior: "auto" });
...
function renderQuestionEvidence(qid) {
  ...
  if (qid === "cardiovascular_components_q03" || qid === "cardiovascular_components_q04" || qid === "cardiovascular_components_q05") return `<div class="evidence-card"><strong>心臟構造判讀</strong><p>請根據題幹與選項中的構造線索作答，不需背完整循環路徑。</p></div>`;
  if (qid === "cardiovascular_components_q06" || qid === "cardiovascular_components_q07") return "";
  if (qid === "cardiovascular_components_q08" || qid === "cardiovascular_components_q09") return "";
  ...
  if (qid === "cardiovascular_components_q13" || qid === "cardiovascular_components_q14") return "";
    ...
    <div class="stack result-stack">
      <section class="panel result-panel">
        ...
        <div class="exp-summary">
          ...
          <span>${statusCopy.summaryLabel}：${estimatedTotal}｜${statusCopy.creditLabel}：${statusCopy.creditValue}</span>
        </div>
...
function resultMode(result = state.result || {}) {
  ...
  if (String(status).includes("server_verified") || status === "verified") return "verified";
  return "pending";
}
