...
const VERSION = "20260731-cell-observation-submitted-retry-ia-v1";
...
function renderResultBadges(result) {
  const earned = new Set(parseArray(result.badges));
  const earnedBadges = badges.filter((badge) => earned.has(badge.id) || earned.has(badge.name));
  if (!earnedBadges.length) {
    const copy = state.student?.is_guest
      ? "guest 測試本次沒有正式取得徽章；正式帳號完成並通過後台確認後才會列入正式累積。"
      : attemptCreditStatus() === "verified"
        ? "本次沒有新增取得的徽章。"
        : "本次徽章結果待後台確認；尚未列入正式累積。";
    return `<section class="result-badges"><h3>本次取得徽章</h3><div class="badge-grid earned-only" data-earned-only="true" data-earned-badge-empty="true"><p class="muted">${copy}</p></div></section>`;
  }
  const statusText = state.student?.is_guest
    ? "guest 測試徽章，不列入正式累積"
    : attemptCreditStatus() === "verified"
      ? "本次正式取得"
      : "本次可能取得，待後台確認";
  return `<section class="result-badges"><h3>本次取得徽章</h3><div class="badge-grid earned-only" data-earned-only="true">${earnedBadges.map((badge) => `
    <div class="badge-card lit earned-now ${badge.id === "cell_observation_flawless" ? "gold" : ""}" data-badge-id="${badge.id}" data-badge-image-path="${badge.badge_image_path}">
      <img class="badge-image" src="${badge.badge_image_path}" alt="${badge.name}">
...
function renderAchievements() {
  return `<div class="wide-layout achievements-stack" data-bq-achievements-overview-only="true"><div class="panel"><p class="eyebrow">再挑戰</p><h2>重新登入後開始新的挑戰</h2><p class="muted">本次作答與結算已鎖定；若要再挑戰，請重新登入並從頭完成。這不會刪除既有正式累積資料。</p><div class="actions"><button class="secondary" id="achieveResult">返回結算</button><button class="secondary" data-relogin-action="true">重新登入，並從登入頁開始</button></div></div><div class="panel bq-all-unit-badge-overview" data-bq-badge-overview="true"></div></div>`;
}
