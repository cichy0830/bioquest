...
function studentForStatus(status) {
  ...
  return {
    ...
    class_name: "722",
    seat_no: "22",
    student_name: status === "verified" ? "內分泌測試生" : "待確認學生",
    profile_gender: "male",
    total_exp: status === "verified" ? 10400 : 10000,
    ...
    progress: {
      ...
      total_exp: status === "verified" ? 10400 : 10000,
      completed_unit_count: status === "verified" ? 22 : 21,
      current_title_id: "systems_investigator",
      title_avatar_path: "../shared-assets/title-avatars/title-06-systems_investigator-male.webp",
  ...
  for (const viewport of [{ width: 1440, height: 900 }, { width: 390, height: 844 }]) {
    for (const status of ["verified", "pending", "guest"]) {
      ...
      await page.addInitScript(({ seededState, attemptsKeyName, stateKeyName, canonicalVersion }) => {
        ...
        localStorage.setItem(attemptsKeyName, JSON.stringify([{ attempt_id: "history_attempt", unit_id: "endocrine_system", unit_credited_exp: 500 }]));
        ...
        window.fetch = async (url, options = {}) => {
          ...
          return { ok: true, json: async () => ({
            ...
            progress: {
              ...
              current_title_id: "systems_investigator",
              title_avatar_path: "../shared-assets/title-avatars/title-06-systems_investigator-male.webp",
              unit_badge_summary_json: JSON.stringify([{ unit_id: "nervous_system", earned_count: 0, total_count: 15, earned_badges: [] }])
            }
          }) };
        };
      ...
      if (status === "verified") {
        assert(achievementState.text.includes("10400 EXP"), "verified title progress must come from StudentProgress");
        assert(achievementState.text.includes("已完成 22 站"), "verified completed units missing");
      }
      if (status === "pending") {
      ...
      const resetState = await page.evaluate(() => ({
        screen: window.__endocrine_systemTest.state().screen,
        student: window.__endocrine_systemTest.state().student,
        attemptId: window.__endocrine_systemTest.state().attempt_id,
        submitted: window.__endocrine_systemTest.state().submitted,
      ...
      if (status === "guest") {
        ...
        assert.equal(resetState.snapshot.student_id, "S22022", `${status} snapshot student`);
        assert.equal(resetState.snapshot.total_exp, status === "verified" ? 10400 : 10000, `${status} snapshot total`);
        await page.locator("#studentId").fill("S22022");
...
console.log("endocrine system submitted retry state regression passed");
