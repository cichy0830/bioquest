...
const root = process.env.BIOQUEST_AUDIT_ROOT
  ...
  : sourceRoot;
const VERSION = "20260813-excretion-water-homeostasis-mapping-v1";
...
function studentForStatus(status) {
  ...
  return {
    ...
    class_name: "725",
    seat_no: "25",
    student_name: verified ? "排泄測試生" : "待確認學生",
    profile_gender: "female",
    total_exp: verified ? 11600 : 11100,
    current_title_id: "systems_investigator",
    title_avatar_path: "shared-assets/title-avatars/title-06-systems_investigator-female.png",
    progress: {
      source: verified ? "server_verified" : "pending_backend",
      progress_applied: verified,
      total_exp: verified ? 11600 : 11100,
      completed_unit_count: verified ? 25 : 24,
      current_title_id: "systems_investigator",
      title_avatar_path: "shared-assets/title-avatars/title-06-systems_investigator-female.png",
      unit_badge_summary_json: JSON.stringify([
        { unit_id: "respiration_homeostasis", earned_count: 0, total_count: 17, earned_badges: [] },
...
function stateForStatus(status, screen = "result") {
  return {
    ...
    result: {
      ...
      earned_badges: ["excretion_water_homeostasis_entry", "excretion_water_homeostasis_flawless"]
    },
    notice: ""
  };
}
  ...
  for (const viewport of [{ width: 1440, height: 900 }, { width: 390, height: 844 }]) {
    for (const status of ["verified", "pending", "guest"]) {
      ...
      await page.addInitScript(({ seededState, attemptsKeyName, stateKeyName, canonicalVersion }) => {
        ...
        localStorage.setItem(attemptsKeyName, JSON.stringify([{ attempt_id: "history_attempt", unit_id: "respiration_homeostasis", unit_credited_exp: 500 }]));
        ...
        window.fetch = async (url, options = {}) => {
          ...
          return { ok: true, json: async () => ({
            ...
            progress: {
              ...
              current_title_id: "systems_investigator",
              title_avatar_path: "shared-assets/title-avatars/title-06-systems_investigator-female.png",
              unit_badge_summary_json: JSON.stringify([{ unit_id: "respiration_homeostasis", earned_count: 0, total_count: 17, earned_badges: [] }])
            }
          }) };
        };
      ...
      assert.equal(await page.locator(".result-stack .badge-visual img").count(), 0, `${status} result should not render pending badge images`);
      ...
      if (status === "verified") {
        assert(achievementState.text.includes("11600 EXP"), "verified title progress must come from StudentProgress");
        assert(achievementState.text.includes("已完成 25 站"), "verified completed units missing");
      }
      if (status === "pending") {
      ...
      const resetState = await page.evaluate(() => ({
        screen: window.__excretion_water_homeostasisTest.state().screen,
        student: window.__excretion_water_homeostasisTest.state().student,
        attemptId: window.__excretion_water_homeostasisTest.state().attempt_id,
        submitted: window.__excretion_water_homeostasisTest.state().submitted,
        attempts: window.__excretion_water_homeostasisTest.loadAttempts().length,
        snapshot: window.__excretion_water_homeostasisTest.loadVerifiedSnapshot(),
      ...
      if (status === "guest") {
        ...
        assert.equal(resetState.snapshot.student_id, "S25025", `${status} snapshot student`);
        assert.equal(resetState.snapshot.total_exp, status === "verified" ? 11600 : 11100, `${status} snapshot total`);
        await page.locator("#studentId").fill("S25025");
...
console.log("excretion water homeostasis submitted retry state regression passed");
