...
function submittedState(mode) {
  ...
  return {
    ...
    attempt_id: `${mode}_u37_attempt`,
    attempt_session_token: `${mode}_u37_token`,
    attempt_session_id: `${mode}_u37_session`,
    ...
    answers: {},
    hints: {},
    reflection: { confident: "資料支持範圍", question: "我想確認化石形態資料能支持到哪個判斷範圍？", confidence: "4" },
    result: {
      ...
      earned_badges: verified ? ["server_u37_badge_without_image"] : ["fossils_evolution_entry", "fossils_evolution_flawless"],
      logs: [],
      reflection: {}
    },
    submitted: true,
    submitLockedAt: "2026-08-18T00:00:00.000Z",
    notice: ""
  };
}
...
async function runCase(browser, mode, viewport) {
  ...
  await page.addInitScript(({ mode, state }) => {
    localStorage.setItem("bioquest_fossils_evolution_state_v1", JSON.stringify(state));
    localStorage.setItem("bioquest_attempts_v1", JSON.stringify([{ unit_id: "fossils_evolution", attempt_id: "historic_attempt", verification_status: "server_verified" }]));
    window.confirm = () => true;
  ...
  const cleanState = await page.evaluate(() => JSON.parse(localStorage.getItem("bioquest_fossils_evolution_state_v1")));
...
try {
  for (const viewport of viewports) {
    ...
    await runEvidenceCase(browser, viewport, "checkpoint2", ".fossil-strata-evidence", ["地層位置資料", "L1", "L3", "未見翻轉或擾動線索"], /L3 通常較早|L1 一定最早|答案是|精確年份/);
    await runEvidenceCase(browser, viewport, "checkpoint4", ".fossil-trait-evidence", ["化石形態資料", "殼紋較少", "殼紋較多"], /演化證明|祖先|後代|精確年份|答案是/);
    await runEvidenceCase(browser, viewport, "checkpoint4", ".fossil-comparison-cards", ["共同特徵比較卡", "分節支撐", "寬尾板", "長尾刺"], /共同祖先|分支關係|答案是/);
    await runEvidenceCase(browser, viewport, "checkpoint3", ".fossil-limit-cards", ["證據資料卡", "三層地層", "觀察範圍"], /不能推論|只能支持|答案是/);
  }
} finally {
  await browser.close();
}

console.log("fossils evolution submitted layout regression passed");
