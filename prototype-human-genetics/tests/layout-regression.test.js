...
function submittedState(mode) {
  ...
  return {
    ...
    attempt_id: `${mode}_u33_attempt`,
    attempt_session_token: `${mode}_u33_token`,
    attempt_session_id: `${mode}_u33_session`,
    ...
    answers: {},
    hints: {},
    reflection: { confident: "匿名資料", question: "我想確認匿名模型資料的可能性，為什麼不能直接判斷真實家庭關係？", confidence: "4" },
    result: {
      ...
      earned_badges: verified ? ["server_u33_badge_without_image"] : ["human_genetics_entry", "human_genetics_flawless"],
      logs: [],
      reflection: {}
    },
    submitted: true,
...
async function runCase(browser, mode, viewport) {
  ...
  await page.addInitScript(({ mode, state }) => {
    localStorage.setItem("bioquest_human_genetics_state_v1", JSON.stringify(state));
    localStorage.setItem("bioquest_attempts_v1", JSON.stringify([{ unit_id: "human_genetics", attempt_id: "historic_attempt", verification_status: "server_verified" }]));
    window.confirm = () => true;
  ...
  const cleanState = await page.evaluate(() => JSON.parse(localStorage.getItem("bioquest_human_genetics_state_v1")));
...
console.log("human_genetics submitted layout regression passed");
