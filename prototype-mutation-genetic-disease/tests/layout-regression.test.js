...
function submittedState(mode) {
  ...
  return {
    ...
    attempt_id: `${mode}_u35_attempt`,
    attempt_session_token: `${mode}_u35_token`,
    attempt_session_id: `${mode}_u35_session`,
    ...
    answers: {},
    hints: {},
    reflection: { confident: "匿名資料", question: "我想確認匿名模型資料能支持到哪個判斷層級？", confidence: "4" },
    result: {
      ...
      earned_badges: verified ? ["server_u35_badge_without_image"] : ["mutation_genetic_disease_entry", "mutation_genetic_disease_flawless"],
      logs: [],
      reflection: {}
    },
    submitted: true,
    submitLockedAt: "2026-08-17T00:00:00.000Z",
    notice: ""
  };
}
...
async function runCase(browser, mode, viewport) {
  ...
  await page.addInitScript(({ mode, state }) => {
    localStorage.setItem("bioquest_mutation_genetic_disease_state_v1", JSON.stringify(state));
    localStorage.setItem("bioquest_attempts_v1", JSON.stringify([{ unit_id: "mutation_genetic_disease", attempt_id: "historic_attempt", verification_status: "server_verified" }]));
    window.confirm = () => true;
  ...
  const cleanState = await page.evaluate(() => JSON.parse(localStorage.getItem("bioquest_mutation_genetic_disease_state_v1")));
...
async function runEvidenceCase(browser, viewport) {
  const page = await browser.newPage({ viewport });
  const errors = [];
  page.on("pageerror", (error) => errors.push(`pageerror:${error.message}`));
  page.on("console", (message) => {
  ...
  const state = {
    ...
    attempt_id: `q07_evidence_${viewport.width}`,
    attempt_session_token: `guest_q07_${viewport.width}`,
    question_version: QUESTION_VERSION,
  ...
  await page.goto(pathToFileURL(path.join(root, "index.html")).href);
  await page.locator(".sex-chromosome-model").waitFor();
  const label = `q07 evidence ${viewport.width}x${viewport.height}`;
  assert.equal(await page.locator(".sex-chromosome-model").count(), 1, `${label}: model card exactly one`);
  assert.equal(await page.locator(".sex-chromosome-model img").count(), 0, `${label}: no bitmap evidence`);
  const text = await page.locator(".sex-chromosome-model").innerText();
  assert(text.includes("性染色體模型卡"), `${label}: title`);
  assert(text.includes("模型代碼"), `${label}: model code label`);
  assert(text.includes("X / Y 性染色體模型"), `${label}: chromosome model`);
  assert(text.includes("染色體上可有遺傳資料片段"), `${label}: genetic data location`);
  assert(!/疾病名稱|病患|患者|家庭|診斷|性別價值|評斷|只出現|某一個性別|機率|答案是|不能診斷|不要責怪|資料不足/.test(text), `${label}: neutral copy`);
...
try {
  for (const viewport of viewports) {
    ...
    await runEvidenceCase(browser, viewport);
  }
} finally {
  await browser.close();
}

console.log("mutation_genetic_disease submitted layout regression passed");
