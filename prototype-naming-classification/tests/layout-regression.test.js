...
function submittedState(mode) {
  ...
  return {
    ...
    attempt_id: `${mode}_u38_attempt`,
    attempt_session_token: `${mode}_u38_token`,
    attempt_session_id: `${mode}_u38_session`,
    ...
    answers: {},
    hints: {},
    reflection: { confident: "分類階層", question: "我想確認正式名稱與俗名在資料整理時怎麼搭配使用？", confidence: "4" },
    result: {
      ...
      earned_badges: verified ? ["server_u38_badge_without_image"] : ["naming_classification_entry", "naming_classification_flawless"],
      logs: [],
      reflection: {}
    },
    submitted: true,
...
async function runCase(browser, mode, viewport) {
  ...
  await page.addInitScript(({ mode, state }) => {
    localStorage.setItem("bioquest_naming_classification_state_v1", JSON.stringify(state));
    localStorage.setItem("bioquest_attempts_v1", JSON.stringify([{ unit_id: "naming_classification", attempt_id: "historic_attempt", verification_status: "server_verified" }]));
    window.confirm = () => true;
  ...
  const cleanState = await page.evaluate(() => JSON.parse(localStorage.getItem("bioquest_naming_classification_state_v1")));
...
async function runHierarchySequenceCase(browser, viewport) {
  const page = await browser.newPage({ viewport });
  const errors = [];
  page.on("pageerror", (error) => errors.push(`pageerror:${error.message}`));
  page.on("console", (message) => {
  ...
  const state = {
    screen: "checkpoint2",
    student: { student_id: "guest", class_name: "測試", seat_no: "00", student_name: "老師測試帳號", is_guest: true },
    attempt_id: `q04_guard_${viewport.width}`,
    attempt_session_token: `guest_${viewport.width}`,
    question_version: QUESTION_VERSION,
    verification_mode: "local_guest",
    completedScreens: ["login", "brief", "scan", "checkpoint1", "checkpoint2"],
    answers: {},
    hints: {},
    reflection: { confident: "", question: "", confidence: "3" },
    result: null,
  ...
  await page.goto(pathToFileURL(path.join(root, "index.html")).href);
  await page.locator('[data-question-id="naming_classification_q04"]').waitFor();
  const label = `q04 hierarchy cards ${viewport.width}x${viewport.height}`;
  assert.equal(await page.locator(".naming-hierarchy-evidence").count(), 0, `${label}: no shared hierarchy evidence table`);
  assert.equal(await page.locator('[data-sequence="naming_classification_q04"] .sequence-item').count(), 7, `${label}: seven sortable cards`);
  assert.equal(await page.locator('[data-move="naming_classification_q04"]').count(), 14, `${label}: up/down controls remain available`);
  const order = await page.locator('[data-sequence="naming_classification_q04"] .sequence-item').evaluateAll((nodes) => nodes.map((node) => node.getAttribute("data-sequence-item")));
  assert.deepEqual([...order].sort(), ["class", "family", "genus", "kingdom", "order", "phylum", "species"], `${label}: exact card set`);
  assert.notDeepEqual(order, ["kingdom", "phylum", "class", "order", "family", "genus", "species"], `${label}: initial card order must not leak canonical sequence`);
  assert.equal(await page.locator('[data-question-id="naming_classification_q05"]').count(), 1, `${label}: q05 remains available`);
  assert.equal(await page.locator(".naming-relationship-evidence").count(), 1, `${label}: q06 neutral comparison evidence remains available`);
  await assertNoBrokenImages(page, label);
  await assertNoHorizontalOverflow(page, label);
  assert.equal(errors.length, 0, `${label}: no console/page errors`);
  await page.close();
...
try {
  for (const viewport of viewports) {
    for (const mode of modes) await runCase(browser, mode, viewport);
    await runEvidenceCase(browser, viewport, "checkpoint1", ".naming-common-name-evidence", ["名稱紀錄卡", "俗名 A", "正式名稱欄"], /答案是|正解|應選/);
    await runEvidenceCase(browser, viewport, "checkpoint1", ".naming-binomial-evidence", ["正式名稱欄位卡", "欄位一", "欄位二"], /答案是|正解|應選/);
    await runHierarchySequenceCase(browser, viewport);
    await runEvidenceCase(browser, viewport, "checkpoint2", ".naming-relationship-evidence", ["分類階層比較表", "A 組", "共同階層代碼"], /答案是|正解|應選/);
    await runEvidenceCase(browser, viewport, "checkpoint3", ".naming-kingdom-evidence", ["五界線索資料", "資料 A", "資料 E"], /答案是|正解|應選/);
    await runEvidenceCase(browser, viewport, "checkpoint4", ".naming-relationship-table", ["分類資料比較表", "甲組", "共同欄位"], /答案是|正解|應選|一定最近/);
    await runEvidenceCase(browser, viewport, "checkpoint4", ".naming-boundary-evidence", ["相鄰單元任務卡", "任務 A", "任務 D"], /答案是|正解|應選/);
    await runEvidenceCase(browser, viewport, "checkpoint4", ".naming-scope-evidence", ["分類資料範圍卡", "資料項目", "觀察範圍"], /答案是|正解|應選|直接證明/);
  }
} finally {
  await browser.close();
}

console.log("naming classification submitted layout regression passed");
