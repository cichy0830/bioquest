...
function studentForStatus(status) {
  ...
  const progressSource = status === "verified" ? "server_verified" : "pending_backend";
  return {
    student_id: "S21021",
    class_name: "721",
    seat_no: "21",
    student_name: status === "verified" ? "神經測試生" : "待確認學生",
    profile_gender: "male",
    total_exp: status === "verified" ? 9800 : 9400,
    ...
    progress: {
      ...
      total_exp: status === "verified" ? 9800 : 9400,
      completed_unit_count: status === "verified" ? 21 : 20,
      current_title_id: "systems_investigator",
      title_avatar_path: "../shared-assets/title-avatars/title-06-systems_investigator-male.webp",
      unit_badge_summary_json: JSON.stringify([
        { unit_id: "stimulus_response", earned_count: 15, total_count: 15, earned_badges: [] },
        { unit_id: "nervous_system", earned_count: 0, total_count: 15, earned_badges: [] }
      ])
    }
  };
}
...
function stateForStatus(status, screen = "result") {
  return {
    ...
    result: {
      ...
      earned_badges: ["nervous_system_entry", "nervous_system_flawless"]
    },
    notice: ""
  };
}
  ...
  for (const viewport of [{ width: 1440, height: 900 }, { width: 390, height: 844 }]) {
    for (const status of ["verified", "pending", "guest"]) {
      ...
      await page.addInitScript(({ seededState, attemptsKeyName, stateKeyName, canonicalVersion }) => {
        ...
        localStorage.setItem(attemptsKeyName, JSON.stringify([{ attempt_id: "history_attempt", unit_id: "nervous_system", unit_credited_exp: 500 }]));
        ...
        window.fetch = async (url, options = {}) => {
          ...
          return { ok: true, json: async () => ({
            ...
            progress: {
              ...
              current_title_id: "systems_investigator",
              title_avatar_path: "../shared-assets/title-avatars/title-06-systems_investigator-male.webp",
              unit_badge_summary_json: JSON.stringify([{ unit_id: "stimulus_response", earned_count: 15, total_count: 15, earned_badges: [] }])
            }
          }) };
        };
      ...
      if (status === "guest") {
        assert(await page.locator(".result-panel").textContent().then((text) => text.includes("不列入正式累積")), "guest result wording must not imply official progress");
      } else if (status === "pending") {
        assert(await page.locator(".result-panel").textContent().then((text) => text.includes("待後台確認")), "pending result wording must wait for backend");
      } else {
        assert(await page.locator(".result-panel").textContent().then((text) => text.includes("後台認列")), "verified result wording must use backend recognition");
      }
      ...
      if (status === "verified") {
        assert(achievementState.text.includes("9800 EXP"), "verified title progress must come from StudentProgress");
        assert(achievementState.text.includes("已完成 21 站"), "verified completed units missing");
      }
      if (status === "pending") {
      ...
      const resetState = await page.evaluate(() => ({
        ...
        snapshot: window.__nervous_systemTest.loadVerifiedSnapshot(),
      ...
      if (status === "guest") {
        ...
        assert.equal(resetState.snapshot.student_id, "S21021", `${status} snapshot student`);
        assert.equal(resetState.snapshot.total_exp, status === "verified" ? 9800 : 9400, `${status} snapshot total`);
        await page.locator("#studentId").fill("S21021");
...
console.log("nervous system submitted retry state regression passed");
