...
async function completeMission(page) {
  ...
  await page.waitForFunction(() => {
    const image = document.querySelector('img[alt="光合作用變因比較示意圖"]');
    return image?.complete && image.naturalWidth > 0;
  });
  const variableControlImage = await page.locator('img[alt="光合作用變因比較示意圖"]').evaluate((img) => ({
    src: img.currentSrc || img.src,
    naturalWidth: img.naturalWidth,
    naturalHeight: img.naturalHeight
  }));
  assert.ok(variableControlImage.src.includes("img-photosynthesis-variable-control-1280w.webp"), "q12 variable-control image must use the 1280w runtime asset");
  assert.ok(variableControlImage.src.includes(`v=${version}`), "q12 variable-control image must include runtime cache");
  assert.equal(variableControlImage.naturalWidth, 1280, "q12 variable-control image natural width");
  assert.equal(variableControlImage.naturalHeight, 720, "q12 variable-control image natural height");
