...
assert.equal(api.VERSION, "20260727-plant-transport-structures-ui-fixes-v1");
...
assert.equal(api.badges.find((badge) => badge.id === "plant_transport_structures_entry")?.image_status, "ready");
assert.equal(api.badges.find((badge) => badge.id === "plant_transport_structures_flawless")?.image_status, "ready");
...
assert(resultHtml.includes("正式圖核准後才會顯示圖像"));
