...
function studentForStatus(status) {
  ...
  return {
    ...
    class_name: "724",
    seat_no: "24",
    student_name: verified ? "呼吸測試生" : "待確認學生",
    profile_gender: "female",
    total_exp: verified ? 11200 : 10800,
    ...
    progress: {
      ...
      total_exp: verified ? 11200 : 10800,
      completed_unit_count: verified ? 24 : 23,
      current_title_id: "systems_investigator",
      title_avatar_path: "../shared-assets/title-avatars/title-06-systems_investigator-female.webp",
...
function stateForStatus(status, screen = "result") {
  return {
    ...
    result: {
      ...
      earned_badges: ["respiration_homeostasis_entry", "respiration_homeostasis_flawless"]
    },
    notice: ""
  };
}
...
try {
  for (const viewport of [{ width: 1440, height: 900 }, { width: 390, height: 844 }]) {
    for (const status of ["verified", "pending", "guest"]) {
      ...
      if (status === "verified") {
        assert(achievementState.text.includes("11200 EXP"), "verified title progress must come from StudentProgress");
        assert(achievementState.text.includes("已完成 24 站"), "verified completed units missing");
      }
      if (status === "pending") {
      ...
      const resetState = await page.evaluate(() => ({
        screen: window.__respiration_homeostasisTest.state().screen,
        student: window.__respiration_homeostasisTest.state().student,
        attemptId: window.__respiration_homeostasisTest.state().attempt_id,
        submitted: window.__respiration_homeostasisTest.state().submitted,
        attempts: window.__respiration_homeostasisTest.loadAttempts().length,
        snapshot: window.__respiration_homeostasisTest.loadVerifiedSnapshot(),
      ...
      if (status === "guest") {
        ...
        assert.equal(resetState.snapshot.student_id, "S24024", `${status} snapshot student`);
        assert.equal(resetState.snapshot.total_exp, status === "verified" ? 11200 : 10800, `${status} snapshot total`);
        await page.locator("#studentId").fill("S24024");
...
console.log("respiration homeostasis submitted retry state regression passed");
