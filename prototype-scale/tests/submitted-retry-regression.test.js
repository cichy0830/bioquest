...
function progressFor(mode) {
  ...
  const scaleSummary = mode === "verified"
    ? {
        sequence: 10,
        unit_id: "scale",
        earned_count: 3,
        ...
        earned_badges: [
          ...
          { badge_id: "scale_reflection_reporter", badge_image_path: "shared-assets/badges/scale/badge-scale-scale_reflection_reporter.webp" }
        ]
      }
    : {
        sequence: 10,
  ...
  return {
    ...
    total_exp: mode === "verified" ? 4817 : 4320,
    completed_unit_count: mode === "verified" ? 10 : 9,
...
function resultFor(mode) {
  ...
  return {
    ...
    question_exp: verified ? 40 : 20,
    question_exp_candidate: verified ? 40 : 20,
    mastery_exp: verified ? 140 : 40,
    retry_exp: 0,
    retry_exp_candidate: 0,
    attempt_total_exp: verified ? 500 : 390,
    attempt_total_exp_candidate: verified ? 500 : 390,
    ...
    reflection_quality: verified ? "discussion_question" : "basic_statement",
    reflection_exp_reason: verified ? "正式回報已認列" : "後台待重算",
    badges: verified
      ? ["scale_entry", "scale_order_sorter", "scale_reflection_reporter"]
      : ["scale_entry", "scale_order_sorter"]
  };
}
...
function submittedState(mode) {
  return {
    ...
    remote_completed_attempts: mode === "guest" ? 0 : 9,
    remote_previous_attempt_id: mode === "guest" ? "" : "u10_previous_attempt",
    remote_previous_accuracy: mode === "guest" ? null : 0.79,
    cumulative_badges: mode === "verified" ? ["scale_entry", "scale_order_sorter", "scale_reflection_reporter"] : [],
    cumulative_total_exp: mode === "verified" ? 4817 : 0,
    completed_unit_count: mode === "verified" ? 10 : 0,
    started_at: "2026-07-31T00:00:00.000Z",
    attempt_id: `u10_${mode}_submitted_attempt`,
    attempt_session_id: `u10_${mode}_submitted_session`,
    attempt_session_token: mode === "guest" ? "" : `u10_${mode}_submitted_token`,
    ...
    answers: {
      q01_sequence: ["backpack", "leaf", "ant", "rice", "cell"],
      q02: {},
      q05: {},
      reflection: {}
    },
    ...
    backend_status: mode === "verified" ? "submitted_verified" : mode === "pending" ? "pending_local" : "local_guest"
  };
}

async function installBackend(page, mode) {
  await page.addInitScript(({ mode, questionVersion }) => {
    window.__u10RetryActions = [];
    ...
    window.fetch = async (rawUrl) => {
      ...
      window.__u10RetryActions.push({ action, url: String(url) });
      if (action === "getStudentAndAttemptStatus") {
        return response({
          ...
          progress: {
            ...
            title_avatar_path: "shared-assets/title-avatars/title-02-life_observer-male.webp",
            unit_badge_summary_json: JSON.stringify([
              { sequence: 10, unit_id: "scale", earned_count: 3, catalog_count: 11, earned_badges: [] }
            ])
          },
          attempt_status: {
            completed_attempt_count: 10,
            previous_attempt_id: "u10_previous_attempt",
      ...
      if (action === "startAttempt") {
        return response({
          ...
          attempt_type: "retry",
          attempt_id: `u10_${mode}_new_attempt`,
          attempt_session_id: `u10_${mode}_new_session`,
          attempt_session_token: `u10_${mode}_new_token`,
          previous_attempt_id: `u10_${mode}_submitted_attempt`,
...
async function inspect(page) {
  ...
  return page.evaluate(() => {
    ...
    return {
      ...
      resultBadgeCards: document.querySelectorAll("[data-result-earned-only='true'] .badge-card").length,
      resultBadgeImages: document.querySelectorAll("[data-result-earned-only='true'] .badge-image").length,
      unitWalls: document.querySelectorAll("[data-bq-unit-achievements], .badge-wall").length,
...
async function runCase(browser, viewport, mode) {
  ...
  const state = submittedState(mode);
  await page.addInitScript((stateSnapshot) => {
    localStorage.clear();
    localStorage.setItem("bioquest_scale_state_v1", JSON.stringify(stateSnapshot));
    localStorage.setItem("bioquest_attempts_v1", JSON.stringify([
      {
        attempt_id: "history_attempt",
        mission: { unit_id: "scale" },
        student: stateSnapshot.student,
  ...
  assert.equal(resultView.resultBadgeCards, mode === "verified" ? 3 : 2, `${mode} ${viewport.width}: result must show only current earned badges`);
  assert.equal(resultView.resultBadgeImages, resultView.resultBadgeCards, `${mode} ${viewport.width}: result earned badges must use real images`);
  ...
  assert.equal(achievementsView.resultBadgeCards, 0, `${mode} ${viewport.width}: achievements must not repeat result earned badges`);
  ...
  assert.equal(lockedNavState.loginDisabled, false, `${mode} ${viewport.width}: submitted login nav must stay usable`);
  assert.equal(lockedNavState.resultDisabled, false, `${mode} ${viewport.width}: submitted result nav must stay usable`);
  assert.equal(lockedNavState.achievementsDisabled, false, `${mode} ${viewport.width}: submitted achievements nav must stay usable`);
  assert.equal(lockedNavState.rulesDisabled, false, `${mode} ${viewport.width}: submitted rules nav must stay usable`);
  ...
  const resetState = await page.evaluate(() => ({
    screen: window.__scaleTest.state().screen,
    student: window.__scaleTest.state().student,
    result: window.__scaleTest.state().result,
    submittedAt: window.__scaleTest.state().submitted_at,
    attemptId: window.__scaleTest.state().attempt_id,
    sessionId: window.__scaleTest.state().attempt_session_id,
    sessionToken: window.__scaleTest.state().attempt_session_token,
  ...
  if (mode === "guest") {
    ...
    await page.locator("#studentIdInput").fill("S70110");
  ...
  const reloginState = await page.evaluate(() => ({
    ...
    sessionId: window.__scaleTest.state().attempt_session_id,
    sessionToken: window.__scaleTest.state().attempt_session_token,
    previousAttemptId: window.__scaleTest.state().remote_previous_attempt_id,
    questionVersion: window.__scaleTest.state().question_version,
    submittedAt: window.__scaleTest.state().submitted_at,
    resultPresent: Boolean(window.__scaleTest.state().result),
    backendStatus: window.__scaleTest.state().backend_status,
    actions: window.__u10RetryActions || []
  }));
  ...
  if (mode === "guest") {
    ...
    assert.equal(reloginState.attemptId, `u10_${mode}_new_attempt`, `${mode} ${viewport.width}: formal relogin must create a new attempt`);
    assert.equal(reloginState.sessionId, `u10_${mode}_new_session`, `${mode} ${viewport.width}: formal relogin must create a new session`);
    assert.equal(reloginState.sessionToken, `u10_${mode}_new_token`, `${mode} ${viewport.width}: formal relogin must create a new token`);
    assert.equal(reloginState.previousAttemptId, `u10_${mode}_submitted_attempt`, `${mode} ${viewport.width}: formal relogin must carry previous attempt id`);
...
(async () => {
  ...
  console.log("scale submitted retry regression passed");
})();
