...
const VERSION = "20260814-sexual-reproduction-mapping-v1";
...
const verifiedSnapshotKey = "bioquest_sexual_reproduction_verified_snapshot_v1";
...
function degroupMappingOrder(question, orderedIds) {
  ...
  const groupCount = new Set(orderedIds.map((id) => mappingGroup(question, id))).size;
  if (groupCount !== 2 && !hasSingleBoundaryMappingGroup(question, orderedIds) && !orderedIds.every((id, index) => id === canonical[index])) return [...orderedIds];
  ...
  if (ids.every((id, index) => id === canonical[index]) && ids.length > 2) {
    for (let shift = 1; shift < ids.length; shift += 1) {
      const rotated = ids.slice(shift).concat(ids.slice(0, shift));
      if (!rotated.every((id, index) => id === canonical[index]) && !hasSingleBoundaryMappingGroup(question, rotated)) return rotated;
    }
    for (let index = 1; index < ids.length; index += 1) {
      const swapped = [...ids];
      [swapped[0], swapped[index]] = [swapped[index], swapped[0]];
      if (!swapped.every((id, orderIndex) => id === canonical[orderIndex]) && !hasSingleBoundaryMappingGroup(question, swapped)) return swapped;
    }
  }
  return ids;
}
...
function renderBrief() {
  ...
  const sceneMedia = assets.briefingSceneHook ? `<picture class="brief-scene-media">${assets.briefingSceneMobileHook ? `<source srcset="${assets.briefingSceneMobileHook}?v=${VERSION}" media="(max-width: 640px)">` : ""}<img class="bq-brief-scene-image" src="${assets.briefingSceneHook}?v=${VERSION}" alt="有性生殖簡報主視覺" onerror="this.closest('.brief-scene-media')?.classList.add('asset-missing')"></picture>` : `<div class="brief-scene-fallback bq-brief-scene-missing" role="img" aria-label="生命延續資料庫場景待接"><strong>生命延續資料庫</strong><span>正式簡報圖核准後，會在此呈現阿澤老師與有性生殖判讀場景。</span></div>`;
...
function renderQuestionEvidence(qid) {
  if (qid === "sexual_reproduction_q04") return `<div class="evidence-card"><strong>事件整理卡</strong><p>先讀每張事件卡的描述，再依事件發生前後整理成合理順序。</p></div>`;
  if (qid === "sexual_reproduction_q07") return `<div class="evidence-card"><strong>情境判讀卡</strong><p>先觀察題目描述的地點、環境與生物類型，再依題目資訊完成配對。</p></div>`;
...
function renderBadgeWall(earned = [], options = {}) {
  ...
  const visibleBadges = options.onlyEarned ? earnedBadges : badges;
  if (options.onlyEarned && visibleBadges.length === 0) {
    return `<section class="panel">
      <p class="eyebrow">本次取得項目</p>
      <h2>本次尚未取得新項目</h2>
      <p class="muted">完成任務後會依本次表現列出實際取得的項目；正式圖尚未核准前不請求不存在的圖檔。</p>
    </section>`;
  }
  return `<section class="panel">
    <p class="eyebrow">${options.onlyEarned ? "本次取得項目" : "本單元項目"}</p>
    <h2>${options.onlyEarned ? "本次實際取得" : "本單元 17 項"}</h2>
    <div class="${options.onlyEarned ? "earned-badge-list" : "badge-wall"}">
      ${visibleBadges.map((badge) => `
        <article class="badge ${earnedSet.has(badge.id) ? "earned" : "locked"}">
          ${badge.image_status === "ready" && badge.badge_image_path
            ? `<div class="badge-visual" data-badge-image-status="ready"><img src="${badge.badge_image_path}?v=${VERSION}" alt="${escapeHtml(badge.name)}" onerror="this.closest('.badge-visual').classList.add('fallback'); this.remove();"></div>`
            : `<code>${escapeHtml(badge.id)}</code>`}
