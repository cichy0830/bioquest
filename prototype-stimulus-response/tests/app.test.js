...
assert.equal(api.VERSION, "20260718-stimulus-response-ready-v1");
...
api.setState({ student: { student_id: "guest", is_guest: true }, attempt_id: "stimulus_valid_reflection", attempt_session_token: "guest", question_version: api.VERSION, answers, reflection: { question: "我想確認同一人連續測五次變快時，怎麼判斷是練習造成，還是疲勞造成？" } });
assert.equal(api.scoreAttempt().unit_credited_exp, 500);

api.setState({ student: { student_id: "guest", is_guest: true }, attempt_id: "hint", attempt_session_token: "guest", question_version: api.VERSION, answers, hints: { [Q(7)]: true }, hintEventStatus: { [Q(7)]: "sent" }, reflection: { question: "我想確認受器和動器如何在同一個情境中分開判斷？" } });
