...
const verifiedSnapshotKey = "bioquest_temperature_glucose_homeostasis_verified_snapshot_v1";
...
const assets = {
  ...
  briefingSceneHook: "assets/temperature-glucose-homeostasis-briefing-azhe-wide.webp",
  briefingSceneMobileHook: "assets/temperature-glucose-homeostasis-briefing-azhe-mobile.webp",
...
function renderBrief() {
  ...
  const sceneMedia = assets.briefingSceneHook
    ? `<picture class="brief-scene-media">
        ...
        <img class="bq-brief-scene-image" src="${assets.briefingSceneHook}?v=${VERSION}" alt="體溫與血糖的恆定簡報主視覺" onerror="this.closest('.brief-scene-media')?.classList.add('asset-missing')">
      </picture>`
...
function renderBadgeWall(earned = [], options = {}) {
  ...
  const badgeVisual = (badge) => badge.image_status !== "ready" || !badge.badge_image_path
    ? `<span class="bq-badge-asset-pending" role="img" aria-label="${escapeHtml(badge.name)}圖像待使用者核准">圖像待核准</span>`
