...
function studentForStatus(status) {
  ...
  return {
    ...
    class_name: "726",
    seat_no: "26",
    student_name: verified ? "恆定測試生" : "待確認學生",
    profile_gender: "female",
    total_exp: verified ? 12100 : 11600,
    current_title_id: "life_researcher",
    title_avatar_path: "../shared-assets/title-avatars/title-07-life_researcher-female.webp",
    progress: {
      source: verified ? "server_verified" : "pending_backend",
      progress_applied: verified,
      total_exp: verified ? 12100 : 11600,
      completed_unit_count: verified ? 26 : 25,
      current_title_id: "life_researcher",
      title_avatar_path: "../shared-assets/title-avatars/title-07-life_researcher-female.webp",
      unit_badge_summary_json: JSON.stringify([
        { unit_id: "excretion_water_homeostasis", earned_count: 0, total_count: 17, earned_badges: [] },
...
function stateForStatus(status, screen = "result") {
  return {
    ...
    hintEventStatus: {},
    submitted: true,
    submitLockedAt: "2026-07-28T00:00:00.000Z",
    ...
    result: {
      ...
      retry_exp: 0,
      attempt_exp: 460,
      unit_credited_exp: status === "verified" ? 460 : 0,
      exp_delta: status === "verified" ? 460 : 0,
      earned_badges: ["temperature_glucose_homeostasis_entry", "temperature_glucose_homeostasis_flawless"]
    },
    notice: ""
  };
}
  ...
  for (const viewport of [{ width: 1440, height: 900 }, { width: 390, height: 844 }]) {
    for (const status of ["verified", "pending", "guest"]) {
      ...
      await page.addInitScript(({ seededState, attemptsKeyName, stateKeyName, canonicalVersion }) => {
        ...
        localStorage.setItem(attemptsKeyName, JSON.stringify([{ attempt_id: "history_attempt", unit_id: "excretion_water_homeostasis", unit_credited_exp: 500 }]));
        ...
        window.fetch = async (url, options = {}) => {
          ...
          return { ok: true, json: async () => ({
            ...
            progress: {
              ...
              total_exp: 12100,
              completed_unit_count: 26,
              current_title_id: "life_researcher",
              title_avatar_path: "../shared-assets/title-avatars/title-07-life_researcher-female.webp",
              unit_badge_summary_json: JSON.stringify([{ unit_id: "excretion_water_homeostasis", earned_count: 0, total_count: 17, earned_badges: [] }])
            }
          }) };
        };
      ...
      assert.equal(await page.locator(".result-stack .badge-visual img").count(), 0, `${status} result must not render pending badge images`);
      assert.equal(await page.locator(".result-stack").textContent().then((text) => text.includes(["徽章", "素材", "待接"].join("")) || text.includes("缺圖")), false, `${status} result has stale pending text`);
      ...
      if (status === "verified") {
        assert(achievementState.text.includes("12100 EXP"), "verified title progress must come from StudentProgress");
        assert(achievementState.text.includes("已完成 26 站"), "verified completed units missing");
      }
      if (status === "pending") {
      ...
      const resetState = await page.evaluate(() => ({
        screen: window.__temperature_glucose_homeostasisTest.state().screen,
        student: window.__temperature_glucose_homeostasisTest.state().student,
        attemptId: window.__temperature_glucose_homeostasisTest.state().attempt_id,
        submitted: window.__temperature_glucose_homeostasisTest.state().submitted,
        attempts: window.__temperature_glucose_homeostasisTest.loadAttempts().length,
        snapshot: window.__temperature_glucose_homeostasisTest.loadVerifiedSnapshot(),
      ...
      if (status === "guest") {
        ...
        assert.equal(resetState.snapshot.student_id, "S26026", `${status} snapshot student`);
        assert.equal(resetState.snapshot.total_exp, status === "verified" ? 12100 : 11600, `${status} snapshot total`);
        await page.locator("#studentId").fill("S26026");
...
console.log("temperature glucose homeostasis submitted retry state regression passed");
